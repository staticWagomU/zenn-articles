...
| カテゴリ | ツール | 設定ファイル | 特徴 |
|---------|--------|-------------|------|
| バージョン管理 | nvm | `.nvmrc` | シェルスクリプト製、最も普及 |
| バージョン管理 | fnm | `.nvmrc`, `.node-version` | Rust製、高速、自動切り替え対応 |
| バージョン管理 | Volta | `package.json` | Rust製、package.jsonでバージョン固定 |
| バージョン管理 | mise | `mise.toml`, `.tool-versions` | Rust製、多言語対応 |
| バージョン管理 | proto | `.prototools` | Rust製、WASM拡張対応 |
| バージョン管理 | asdf | `.tool-versions` | Go製、プラグイン方式で多言語対応 |
| バージョン管理 | n | `.node-version` | シェルスクリプト製、npmで導入可能 |
| Node.js組み込み | Corepack | `package.json` | Node.js同梱、パッケージマネージャー管理 |
| Node.js組み込み | Dev Engines | `package.json` | Node.js 22+、ランタイム自動取得 |
| 環境構築 | direnv + nvm | `.envrc` | ディレクトリ移動で自動切り替え |
| 環境構築 | devenv | `devenv.nix` | Nix基盤、宣言的な環境構築 |
| コンテナ | Docker | `Dockerfile` | コンテナ分離、再現性が高い |
| コンテナ | Dev Containers | `devcontainer.json` | VSCode/IDE統合、開発環境の共有 |
