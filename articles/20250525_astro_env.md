...
cloudflare workesのビルドコマンドに`npm astro --config astro.config.cf.mjs build`とすれば何も面倒なことは必要なかったのですが、一つのconfigファイルを環境毎に分岐させたいと考え、環境変数を使うことによって分岐させることに成功したため、ブログとして残しておきます。
